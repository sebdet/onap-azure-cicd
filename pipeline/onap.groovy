node {
   ...
   stage('Build New Docker') {
        ...
        checkout([$class: 'GitSCM', branches: [[name: 'master']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'onap-azure-cicd']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'github-key-cicd-project', url: 'git@github.com:sebdet/onap-azure-cicd.git',name: 'pipeline_project']]])  
        checkout([$class: 'GitSCM', branches: [[name: 'FETCH_HEAD']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: '$GERRIT_PROJECT']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'lf-key-onap-bot', refspec: '$GERRIT_REFSPEC', url: 'ssh://OnapTesterBot@$GERRIT_HOST:$GERRIT_PORT/$GERRIT_PROJECT', name: 'onap_project']]])
