node {
   ...
   stage('Build New Docker') {
    ...
    stage('Run Tests For Component') {
        ...
        TEST_STATUS = sh(returnStatus: true, script: "bash scripts/testing/so/run-tests.sh -d /var/lib/jenkins/workspace/deploy-component/oom/kubernetes -o ./test-results/$GERRIT_CHANGE_NUMBER-$GERRIT_PATCHSET_NUMBER")
        if (TEST_STATUS != 0) {
            
            // For SSH private key authentication, try the sshagent step from the SSH Agent plugin.
            sshagent (credentials: ['github-key-cicd-project']) {
                sh("git add ./test-results/$GERRIT_CHANGE_NUMBER-$GERRIT_PATCHSET_NUMBER/*")
